...
public class AdminPossessionCardDAOImpl extends GenericDao implements AdminPossessionCardDAO{
	...
	public Boolean checkPermissionEdit(Integer userId, Integer cardId){
		Query query = getEntityManager().createNativeQuery("SELECT * FROM admin_possession_card pc WHERE pc.user_id = :userId AND"
				+ " pc.card_id = :cardId ");
	...
	public Integer resetAllocationTeam(Integer teamId){
		Query query = getEntityManager().createNativeQuery("DELETE FROM admin_possession_card "
