		...
		<div class="box-1">
			...
			<div class="content-box-1">
				...
				<div class="box-3">
					...
					<c:if
						test="${permissionType.downloadGroup == 1 && permissionType.groupDataDlRequestFlg != 1}">
					...
					<c:if
						test="${permissionType.downloadAll == 1  && permissionType.allDataDlRequestFlg != 1}">
						<button id="3" class="export">グループの名刺をダウンロード</button>
					</c:if>
					<c:if
						test="${permissionType.downloadAll == 1 && permissionType.allDataDlRequestFlg == 1}">
						<a href="downloadCSV/3" class="export">グループの名刺をダウンロード</a>
					</c:if>

					<input class="hidden" name="groupAll"
						value="${permissionType.allDataDlRequestFlg}"> <input
						class="hidden" name="groupDownload"
						value="${permissionType.groupDataDlRequestFlg}">
				</div>
				<p class="p-line"></p>
				...
				<table class="div-gird">
					...
					<c:forEach var="downloadCSVHistory" items="${downloadCSVHistory}"
						varStatus="loop">
						<tr>
							<td><fmt:formatDate
									value="${downloadCSVHistory.requestDate}" pattern="dd-MM-yyyy" /></td>
							<td><c:out value="${downloadCSVHistory.csvType}" /></td>
							<td><fmt:formatDate
									value="${downloadCSVHistory.approvalDate}" pattern="dd-MM-yyyy" /></td>
							<td><a href="downloadFileCSV/${downloadCSVHistory.csvUrl}"><c:out
										value="${downloadCSVHistory.csvUrl}" /></a></td>
						</tr>
					</c:forEach>
				</table>
