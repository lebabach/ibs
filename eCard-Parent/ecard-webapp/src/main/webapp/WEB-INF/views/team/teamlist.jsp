    ...
    <div class="row bg-white box-shadow menu-top-header">
    	<div class="col-sm-12">
        	<div class="float-left ">
            	<h4 class="h4-header"><fmt:message key="team.list.title"/></h4>
            </div>
            <div class="float-right">
				<a id="add" href="<c:url value='/teams/register'/>"><i class="fa fa-plus icon-rounded"></i></a>
			</div>
        </div>
    </div>
...
<div class="col-sm-12 container table-list-operator">
	<div class="row " id="data-table">
		<div class="ibox-content ibox-content-company">
			<table class="table container dataTable no-footer" id="teamlist" style="margin-top: -84px;" role="grid" aria-describedby="paging_info">
				<thead>
					<tr role="row">
						...
						<th><fmt:message key="team.action"/></th></tr>
				</thead>
				<tbody>
				<c:forEach var="team" items="${teamlistVO}" varStatus="loop">
					<tr id="${loop.index}" role="row" class="${loop.index%2==0?'odd':'even'}">
						...
						<td class="ch-color-link">
							<a class="ch-ok" href="<c:url value='/teams/display/${team.teamId}'/>"><fmt:message key = "team.button.display" /></a>
							<a class="ch-edit" href="<c:url value='/teams/edit/${team.teamId}'/>"><fmt:message key = "operator.list.edit" /></a>
							<a class="ch-add" href="<c:url value='/teams/add-team-member/${team.teamId}'/>"><fmt:message key = "team.list.button.add" /></a>
