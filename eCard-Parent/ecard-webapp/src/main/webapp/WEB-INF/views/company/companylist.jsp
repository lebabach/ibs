                    ...
                    <div class="float-right float-right-manage">
                    	...
                    	<!--  <button id="btn_add_company" class="btn btn-primary  btn_manage" type="button">                  
                         ...
                         <c:if test="${pageContext.request.isUserInRole('ROLE_ADMIN') }">
                            <span><a href='<c:url value="/companies/add"/>' class="btn btn-primary btn_cancle" data-dismiss="modal">新規追加</a></span>
                            </c:if>
                    </div>
                </div>
            </div>
				...
				<div class="col-sm-12 container table-list-operator">
					<div class="row " id="data-table">
						<div class="ibox-content ibox-content-company">
							<table class="table container" id="listcompany" style="margin-top: -84px;">
								<thead>
									<tr>
								...
								<c:forEach var="companyObject" items="${lstcompanyDisplayVO}" varStatus="loop">
									<tr id="${loop.index}" role="row" >
										...
										<td><fmt:formatDate value="${companyObject.createDate}" pattern="yyyy年 MM月dd日" /></td>
										<td class="ch-color-link">
	                                        <a href='<c:url value="/companies/confirm/${companyObject.groupCompanyId}"/>' class="ch-ok"><fmt:message key="company.action.view"/></a>

	                                        <a href='<c:url value="/companies/edit/${companyObject.groupCompanyId}"/>' class="ch-edit"><fmt:message key="company.action.edit"/></a>
	                                       ...
	                                       <%--  <a href='<c:url value="/companies/adddepartment/${companyObject.groupCompanyId}"/>' class="ch-scv"><fmt:message key="company.action.csv"/></a> --%>
	                                        ...
	                                        <a class="ch-del"><fmt:message key="company.action.delete"/></a>
	                                    </td>
									</tr>
								</c:forEach>
								</tbody>
