    ...
    <div id="center-side" class="col-sm-12">
        ...
        <div class="row bg-white box-shadow menu-top-header">
            <div class="col-sm-12 ch-check" id="ct1">
                <div class="float-left col-sm-6">
                    <h4 class="h4-header"><fmt:message key="msg.business.card.data.migration"/></h4>
        ...
        <div class="row bg-white box-shadow box-marginTop5 padding-top-bottom col-sm-12" >
           <div style="margin: 0 auto">
	            ...
	            <form class="form-horizontal" role="form" id="upload" action = "<c:url value="/data/uploadUserCSV"/>" enctype="multipart/form-data" method="POST">
           		<!-- Import User -->
           		
                 	<div class="col-md-3 col-xs-offset-4">
                      <select id="groupId" name="groupCompanyId" style=" width: 170px;height:32px;">
	                      <option value=''><fmt:message key="msg.company.selection"/></option>
                      ...
                      <input type="text" name="groupCompanyId" id="groupCompanyId" class="hidden" value="">
                     <div class="col-md-4">
					  <select id="userId" name="userId" style=" width: 170px;height:32px;">
	                      <option value=''><fmt:message key="msg.user.selection"/></option>
	                      <c:forEach var="listUserInfo" items="${listUserInfo}" varStatus="loop">
	                      	<option value = '${listUserInfo.userId}'><c:out value="${listUserInfo.name}"/></option>                             
	                      </c:forEach>                             
                      </select>
                      <input type="text" name="userId" id="userId" class="hidden" value="">
                    </div>  	
           
           		<div class="col-md-3 col-xs-offset-4">
	                 <div class="form-group" style="margin-top: 7px;margin-left: 12px;">
		                 <div class="row">
...
<%--                     <c:if test="${msgImportSuccess != null}">
                    ...
                    <form id="frmImportCsv" name="frmImportCsv" style="padding-bottom: 10px;" method="POST" action="<c:url value="/data/uploadCardCsv"/>" enctype="multipart/form-data">
                        <div class="form-group" style="margin-top: 7px; margin-left: 12px;">
                            <div class="row">       
                                <label for="fileCSV" class="control-label">名刺CSVファイル選択</label>
                                <input type="file" name="file" class="submit-1" id="file" />
                                
                                <input type="text" name="hGroupCompanyId" id="hGroupCompanyId" class="hidden" value="">
                                <input type="text" name="huserId" id="huserId" class="hidden" value="">
                            </div>
                            <div class="row" style="margin-top:5px;">
			                 	<span><button type="button" class="btn btn-primary" id="btnImportCardCsv"><fmt:message key="team.allocation.register"/></button></span>
			                </div>
                        </div>
                    </form>
                </div>
