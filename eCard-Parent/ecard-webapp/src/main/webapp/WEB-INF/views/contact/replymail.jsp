		...
		<div id="right-side" class="col-sm-12">
			...
			<div
				...
				<div class="col-sm-12 container table-list-operator">
					<form method="POST" action="<c:url value='/contacts/reply'/>"
						accept-charset="UFT-8" id="formSubmit">
						<div class="row " id="data-table">
							<div class="ibox-content  ibox-content-question ibox-custom01">
								<table class="table container paging"
									...
									<tbody>
										<tr>
											<c:choose>
												...
												<c:otherwise>
													<td><c:out value="${contactNotification.lastName}" />
														<c:out value="${contactNotification.firstName}" /></td>
												</c:otherwise>
											</c:choose>
											...
											<td><fmt:formatDate
													value='${contactNotification.createDate}'
								...
								<td>
									<div style="margin-top: 0px; margin-left: 0px;">
										<textarea class="form-control" rows="10" disabled><c:out
												value="${contactNotification.inquiryText}" /></textarea>
										<input type="hidden" name="inquiryId"
											value="${contactNotification.inquiryId}"> <input
											type="hidden" name="email"
											value="${contactNotification.email}">
									</div>
								</td>
							</tr>
						</table>
					...
					<div class="col-xs-12" style="margin-top: 30px;">
						<table style="width: 100%">
									...
									<div style="margin-top: 0px; margin-left: 0px;">
										<c:if test="${contactNotification.answerFlg == 1}">
											<textarea class="form-control" rows="10" disabled
												name="answerText" id="answerText"><c:out
													value="${contactNotification.answerText}" /></textarea>
										</c:if>
										<c:if test="${contactNotification.answerFlg != 1}">
											<textarea class="form-control" rows="10" name="answerText"
							...
							<tr>
								<td></td>
								<td align="right"><c:if
										test="${contactNotification.answerFlg != 1}">
										<div style="margin-top: 20px; margin-left: 10px;">
											<button type="button" class="btn btn-primary btn_add"
												style="width: 150px !important">回答送信</button>
										</div>
									</c:if> <c:if test="${contactNotification.answerFlg == 1}">
										<div style="margin-top: 20px; margin-left: 10px;">
											<button type="button" class="btn btn-primary btn_back"
												style="width: 150px !important">戻る</button>
										</div>
									</c:if></td>
							</tr>
						</table>
