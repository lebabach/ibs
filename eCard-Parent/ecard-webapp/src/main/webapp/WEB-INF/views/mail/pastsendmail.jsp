	...
	<div class="container-fluid padding-top20 bg-container height100per">
		...
		<div id="right-side" class="col-sm-12">
			...
			<div
						...
						<div class="ibox-content   ibox-custom01">
							<table class="table container paging" id="paging"
								...
								<thead>
									<tr>
										<td colspan="5"
											style="background-color: #fff; padding-left: 0;">過去送信メール</td>
									</tr>
									<tr>
										<th>送信日時</th>
										<th>題目</th>
								...
								<tbody>
									<c:forEach var="showListHistory" items="${showListHistory}"
										varStatus="loop">
										<tr id="${showListHistory.id}">
											<td style="width: 25%"><fmt:formatDate
													value="${showListHistory.createDate}"
													pattern="yyyy-MM-dd HH:mm:ss" /></td>
											<td style="width: 50%"><c:out
													value="${showListHistory.mailTitle}" /></td>
											<td style="width: 25%"><input type="hidden"
												name="sendType" value="${showListHistory.sendType}" /> <input
												type="hidden" name="mailDetail"
												value="${showListHistory.mailDetail}" /> <input
												type="hidden" name="mailTitle"
												value="${showListHistory.mailTitle}" /> <input type="hidden"
												name="userSendTo" value="${showListHistory.userSendTo}" /> <input
												type="hidden" name="userId"
												value="${showListHistory.userId}" /> <c:if
													test="${showListHistory.sendType==1}">
													<c:out value="全送信" />
												</c:if> <c:if test="${showListHistory.sendType==2}">
													<c:out value="${showListHistory.userSendTo}" />
												</c:if> <c:if test="${showListHistory.sendType==3}">
													<c:out value="${showListHistory.userSendTo}" />
												</c:if></td>
										</tr>
									</c:forEach>
										...
										<div class="i-checks">
											<label class="">
												<div class="icheckbox_square-green"
													...
													<input type="checkbox" disabled name="selectGroup"
														style="position: absolute; opacity: 0;">
													<ins class="iCheck-helper"
														style="position: absolute; top: 0%; left: 0%; display: block; width: 100%; height: 100%; margin: 0px; padding: 0px; border: 0px; opacity: 0; background: rgb(255, 255, 255);"></ins>
												</div> <i></i> 会社別送信
				...
				<div class="col-sm-12 list-user container table-list-operator"
					style="margin-top: -30px;">
					<div class="row " id="data-table-user-list">
						<div class="ibox-content   ibox-custom01">
							<table class="table container paging" id="paging_1"
							...
							<tr>
								...
								<td>
									<div style="margin-left: 10px;">
										<input disabled id="showMailTitle" type="text" placeholder=""
