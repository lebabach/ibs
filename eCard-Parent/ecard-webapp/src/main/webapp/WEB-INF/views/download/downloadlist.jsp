	...
	<div class="row bg-white box-shadow menu-top-header">
		<div class="col-sm-12">
			<div class="float-left">
				<h4 class="h4-header">
					<fmt:message key="download.header" />
				</h4>
			</div>
			...
			<div class="row clearfix">
				...
				<div class="row " id="data-table">
					<div class="ibox-content ibox-custom01">
						...
						<table id="first-table" class="table container paging"
							style="padding: 0;">
							<thead style="">
								<tr>
									<td colspan="7"
										style="background-color: #fff; padding-left: 0;">自社名刺ダウンロード申請者</td>
								</tr>
								<tr>
									<th></th>
							...
							<tbody style="">
								<c:forEach var="downloadGroupCSVHistory"
									items="${downloadGroupCSVHistory}" varStatus="loop">

									<tr>
										<td>
											<div class="i-checks"
												data-check-id="${downloadGroupCSVHistory.csvId}">
												...
												<c:if test="${downloadGroupCSVHistory.csvApprovalStatus!=0}">
													<label class="">
														<div class="icheckbox_square-green checked disabled"
															...
															<input type="checkbox"
																value="${downloadGroupCSVHistory.csvId}" checked
																disabled style="position: absolute; opacity: 0;">
												...
												<c:if test="${downloadGroupCSVHistory.csvApprovalStatus==0}">
													<label class="">
														<div class="icheckbox_square-green"
															style="position: relative;">
															<input type="checkbox"
																value="${downloadGroupCSVHistory.csvId}"
											...
											</div> <%-- <input class="hidden" name="csvId" value="${downloadGroupCSVHistory.csvId}"> --%>
										</td>

										<td><c:out value="${downloadGroupCSVHistory.userName}" /></td>
										<td><c:out
												value="${downloadGroupCSVHistory.departmentName}" /></td>
										<td><c:out
												value="${downloadGroupCSVHistory.positionName}" /></td>
										<td><c:if test="${downloadGroupCSVHistory.operaterId!=0}">
												<c:out value="${downloadGroupCSVHistory.operaterName}" />
											</c:if></td>

										<td><c:if
												test="${downloadGroupCSVHistory.csvApprovalStatus==0}">
												<c:out value="未承認" />
											</c:if> <c:if test="${downloadGroupCSVHistory.csvApprovalStatus==1}">
												<c:out value="承認" />
											</c:if> <c:if test="${downloadGroupCSVHistory.csvApprovalStatus==2}">
												<c:out value="否認" />
											</c:if> <input class="hidden" name="csvApprovalStatus"
											value="${downloadGroupCSVHistory.csvApprovalStatus}">
										</td>
										<td><fmt:formatDate
												value="${downloadGroupCSVHistory.requestDate}"
						...
						<div style="overflow: hidden; margin: 0 auto; width: 210px;">
							<div style="float: right;">
								<button id="ok-01" type="button" class="btn btn-primary"
									style="display: inline-block; width: 100px !important">承認</button>
								<button id="ng-01" type="button" class="btn btn-primary"
									style="display: inline-block; width: 100px !important">否認</button>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="row " id="data-table">
					<div class="ibox-content ibox-custom01">
						<table id="second-table" class="table container paging"
							style="padding: 0;">
							<thead>
								<tr>
									<td colspan="7"
										style="background-color: #fff; padding-left: 0;">グループ名刺ダウンロード申請者</td>
								</tr>
								<tr>
									<th></th>
									<th>申請者</th>
							...
							<tbody style="">
								<c:forEach var="downloadAllCSVHistory"
									items="${downloadAllCSVHistory}" varStatus="loop">
									<form id="${downloadAllCSVHistory.csvId}" method="POST"
										action="/ecard-webapp/download/regconitionDownload/${downloadAllCSVHistory.csvId}"
										accept-charset="UFT-8">
										<tr>
											<td><div class="i-checks" data-check-id="1">
													<label class="">
														<div class="icheckbox_square-green"
															style="position: relative;">
											...
											<td><c:out
													value="${downloadAllCSVHistory.departmentName}" /></td>
											<td><c:out value="${downloadAllCSVHistory.positionName}" /></td>
											<td><c:if test="${downloadAllCSVHistory.operaterId!=0}">
													<c:out value="${downloadAllCSVHistory.operaterId}" />
												</c:if></td>

											<td><c:if
													test="${downloadAllCSVHistory.csvApprovalStatus==0}">
													<c:out value="未承認" />
												</c:if> <c:if test="${downloadAllCSVHistory.csvApprovalStatus==1}">
													<c:out value="承認" />
												</c:if> <c:if test="${downloadAllCSVHistory.csvApprovalStatus==2}">
													<c:out value="否認" />
												</c:if> <input class="hidden" name="csvApprovalStatus"
												value="${downloadAllCSVHistory.csvApprovalStatus}">
											</td>
											<td><fmt:formatDate
													value="${downloadAllCSVHistory.requestDate}"
													pattern="yyyy年 MM月dd日" /></td>
										</tr>
									</form>
								</c:forEach>
						...
						<div style="overflow: hidden; margin: 0 auto; width: 210px;">
							<div style="float: right;">
								<button id="ok-02" type="button" class="btn btn-primary"
									style="display: inline-block; width: 100px !important">承認</button>
								<button id="ng-02" type="button" class="btn btn-primary"
									style="display: inline-block; width: 100px !important">否認</button>
							</div>
						</div>
					</div>
